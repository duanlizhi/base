 ...
 * @package com.xcj.common.redis  
 * @fileName com.xcj.*
 * @createDate Apr 16, 2014 11:05:46 AM 
 * @author su_jian 
 */
public class TestMain {
	
	 /**
	   * <b>function:</b> 调用方法DEMO
	   * @project base_frame
	   * @package com.xcj.common.redis  
	   * @fileName com.xcj.*** 
	   * @createDate Apr 16, 2014 3:20:01 PM
	   * @return void
	   * @author su_jian
	   */
	 public static void main(String[] args) {
		...
		/* redisUtil.jedis.flushDB();
		 for (int i = 0; i < 200; i++) {
			 redisUtil.shardedJedis.set("key::"+i, "value::"+i);
		 }
		 redisUtil.jedis.save();
		 redisUtil.jedisPool.returnResource(redisUtil.jedis);
		 redisUtil.shardedJedisPool.returnResource(redisUtil.shardedJedis);
		 System.out.println("操作结束");*/
		 //redisUtil.jedis.flushDB();
		 System.out.println("系统中所有键如下：");
         Set<String> keys = redisUtil.jedis.keys("*"); 
         Iterator<String> it=keys.iterator() ;   
         while(it.hasNext()){   
             String key = it.next();   
             System.out.println("key::"+key+"值::"+redisUtil.jedis.get(key));
         }
         System.out.println("取成功");
	 ...
	 /*private void saveT1est() {
		 System.out.println("测试信息如下--------------------------");
		 redisUtil.jedis.flushDB();
		 redisUtil.jedis.expire("key001", 65536002);
		 for (int i = 0; i < 1000000; i++) {
			 shardedredisUtil.jedis.set("key2"+i, "value2"+i);
		 }
		 jedis.save();
         ...
         while(it.hasNext()){   
             String key = it.next();   
             System.out.println("值::"+jedis.get(key));
