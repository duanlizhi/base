...
//频繁提交学习数据 连分钟提交一次
content.study.commitStudy=function (token,courseNumber,userEmail,domainUsername,sessionId,receive,character,progress,score,isComplete,isPass,actionTime){
	if(!content.pub.isAvailable){return false;}
	var objData = {
		token:token,
		courseNumber:courseNumber,
...
//学习退出
content.study.exitStudy=function (token,courseNumber,userEmail,domainUsername,sessionId,receive,character,progress,score,isComplete,isPass,actionTime){
	if(!content.pub.isAvailable){return false;}
	//退出后初始化为空
	Htoken="";
	HcourseNumber="";
